...
function validateForm() {
    ...
    for (i = 0; i < y1.length; i++) {
        ...
        if ((y1[i].name === "studentEmail") && (!checkPatternEmail())) {
        ...
        if ((y1[i].name === "academicYear") && (!checkPatternAcademicYear())) {
            console.log("ESTOY DENTRO VERIFICAR PATTERN CURSO ACAD");
            // add an "invalid" class to the field:
            y1[i].className += " invalid";
            // and set the current valid status to false
            valid = false;
...
function configureDropDownUniversityList(selectedUniversity) {
    ...
    for (var i = 0; i < estudiosJSON.length; i++) {
        // Comprueba si ya existe esta universidad en el array para no mostar duplicados
        var universidad = estudiosJSON[i].centro.universidad;
        var existeUniversidad = universidadesDePaisSeleccionado.some(function(universidadGuardada) {
            console.log("univ guardada: " + universidadGuardada.nombre);
            console.log("univ: " + universidad.nombre);
            return universidadGuardada.nombre === universidad.nombre;
        });
        
        if ((estudiosJSON[i].centro.universidad.localizacion.pais === countrySelect.value) && (!existeUniversidad)) {
...
function configureDropDownCenterList(selectedCenter) {
    ...
    for (var i = 0; i < estudiosJSON.length; i++) {
        // Comprueba si ya existe este centro en el array para no mostar duplicados
        var centro = estudiosJSON[i].centro;
        var existeCentro = centrosDeUniversidadSeleccionada.some(function(centroGuardado) {
            return centroGuardado.nombre === centro.nombre;
        });
        
        if ((estudiosJSON[i].centro.universidad.id == universitySelect.value) && (!existeCentro)) {
    ...
    configureDropDownStudyList(''); // Se resetean los valores del dropdownlist de estudios cada vez que se cambia el dropdownlist de centros
}
...
function configureDropDownStudyList(selectedStudy) {
    ...
    console.log("CENTRO ID IN STUDY: " + centerSelect.value);
    for (var i = 0; i < estudiosJSON.length; i++) {
        // Comprueba si ya existe este estudio en el array para no mostar duplicados
        var estudio = estudiosJSON[i];
        var existeEstudio = estudiosDeCentroSeleccionado.some(function(estudioGuardado) {
            return estudioGuardado.nombreEstudio === estudio.nombreEstudio;
        });
                
        if ((estudiosJSON[i].centro.id.idCentro == centerSelect.value) && (estudiosJSON[i].centro.universidad.id == universitySelect.value) &&
                (!existeEstudio)) {
            console.log("ESTUDIO: " + estudiosJSON[i].nombreEstudio);
